...
try {
  ...
  const toggle = ev(`app.setting.open();app.setting.openTabById('obsidian-style-settings');${delay}const doc=app.setting.win.document;doc.querySelector('.style-settings-heading[data-id="lanternwood"].is-collapsed .style-settings-collapse-indicator')?.click();${delay}doc.querySelector('.style-settings-heading[data-id="lw-landscape"].is-collapsed .style-settings-collapse-indicator')?.click();${delay}const row=[...doc.querySelectorAll('.setting-item')].find(e=>e.querySelector('.setting-item-name')?.textContent==='Activar el bosque');if(!row)throw Error('Forest toggle missing');row.querySelector('.checkbox-container').click();${delay}return document.body.classList.contains('lw-forest');`);
  assert.equal(toggle, !Boolean(original.forest));
  ev(`const row=[...app.setting.win.document.querySelectorAll('.setting-item')].find(e=>e.querySelector('.setting-item-name')?.textContent==='Activar el bosque');row.querySelector('.checkbox-container').click();${delay}return true;`);
